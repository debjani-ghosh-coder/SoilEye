...
if __name__=='__main__':
    ...
    low = 0
   # Open the file for writing the latest percentage value.
    with open("/home/pi/Documents/Debjani/UPS_HAT_D/latest_percentage.txt", "w") as percent_file:
      while True:
        ...
        if(p > 100):p = 100
        if(p < 0):p = 0
        # Write only the latest percentage to the file.
        percent_file.seek(0)  # Move to the beginning of the file.
        percent_file.write(f"{p:.1f}\n")  # Write the latest percentage.
        percent_file.flush()  # Ensure the data is written immediately.

        # INA219 measure bus voltage on the load side. So PSU voltage = bus_voltage + shunt_voltage
        #print("PSU Voltage:   {:6.3f} V".format(bus_voltage + shunt_voltage))
        #print("Shunt Voltage: {:9.6f} V".format(shunt_voltage))
        print("Load Voltage:  {:6.3f} V".format(bus_voltage), flush=True)
        sys.stdout.flush()
        print("Current:       {:6.3f} A".format(current/1000), flush=True)
        sys.stdout.flush()
        print("Power:         {:6.3f} W".format(power), flush=True)
        sys.stdout.flush()
        print("Percent:       {:3.1f}%".format(p), flush=True)
        #print("{:3.1f}%".format(p), flush=True)
        sys.stdout.flush()
